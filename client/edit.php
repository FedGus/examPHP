<?php 
    ...
    $sql_fio = mysqli_query($mysqli, 'SELECT * FROM client WHERE id_client = '.$_SESSION['id'].';');
   
    while ($row = mysqli_fetch_assoc($sql_fio)) {
    $sql_res = mysqli_query($mysqli, 'SELECT * FROM zakaz_new WHERE client="'.$row['FIO'].'" ORDER BY id_order');
    if (!mysqli_errno($mysqli)) {
        ...
        echo '</select><br><label for="tiraz">Тираж (количество):</label><input name="tiraz" type="number" id="tiraz" min="0" max="';
         ...
         echo '><br><label for="status">Статус:</label><span>';
        if ($currentROW) {
                echo $currentROW['status'];
            } 
        echo '</span><br><input type="hidden" name="id" value=" ';
        if ($currentROW) {
