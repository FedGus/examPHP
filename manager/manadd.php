...
<?php
    if ((isset($_POST['btn']) && ($_POST['btn']='Оформить заказ'))) {
        ...
        $sql_res = mysqli_query($mysqli, 'INSERT INTO zakaz (client, color, format, density, tiraz, status) VALUES ("'.htmlspecialchars($_POST['client']).'", "'.htmlspecialchars($_POST['color']).'", "'.htmlspecialchars($_POST['format']).'", "'.htmlspecialchars($_POST['thickness']).'", "'.htmlspecialchars($_POST['quantity']).'", "1");');
...
?>
<form method="POST" action="manindex.php?p=add">
            <h2 class="row">Введите параметры заказа</h2>
            <br><br>
            <?php 
            echo '<label for="density">Клиент:</label><select name="client" id="client">';
            
                $sql_fio = mysqli_query($mysqli, 'SELECT id_client, FIO FROM client;');
                while ($row = mysqli_fetch_assoc($sql_fio)) {
